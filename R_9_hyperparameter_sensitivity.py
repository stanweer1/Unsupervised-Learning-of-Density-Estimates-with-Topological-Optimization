...
BENCHMARK_SUITE = {
    '1D_bimodal': {'data_func': lambda n: data_gen.generate_samples_1D(data_gen.bimodal_gaussian_pdf, n_samples=n),
                   'pdf_func': data_gen.bimodal_gaussian_pdf, 'dims': 1, 'n_samples': 5000},
    '1D_complex': {'data_func': lambda n: data_gen.generate_samples_1D(data_gen.complex_mixture_pdf, n_samples=n),
                   'pdf_func': data_gen.complex_mixture_pdf, 'dims': 1, 'n_samples': 5000},
    '2D_weibull': {'data_func': lambda n: data_gen.generate_samples_2D(data_gen.weibull_pdf, n_samples=n),
                  'pdf_func': data_gen.weibull_pdf, 'dims': 2, 'n_samples': 2000}
}

# ===================================================================
# SINGLE JOB FUNCTION
# ===================================================================
def run_single_job(dataset_name, seed, count_on, TP_on, ref_on, grid_per_dim):
    print(f"\n--- Job: {dataset_name} | seed={seed} | count={count_on} | TP={TP_on} | reg={ref_on} ---")
    ...
    output_dir = "hyperparameter_sensitivity"
    os.makedirs(output_dir, exist_ok=True)
    
    np_save_path = os.path.join(
        output_dir,
        f"{dataset_name}_seed_{seed}_{count_on}_{TP_on}_{ref_on}_{grid_per_dim}.npy"
    )
...
if __name__ == "__main__":
    if len(sys.argv) != 7:
        print("Usage: python 5_run_job.py <dataset_name> <seed> <count_on> <TP_on> <ref_on> <grid_per_dim>")
        sys.exit(1)
    
    dataset_name = sys.argv[1]
    seed = int(sys.argv[2])
    count_on = float(sys.argv[3])
    TP_on = float(sys.argv[4])
    ref_on = float(sys.argv[5])
    grid_per_dim = int(sys.argv[6])
    
    run_single_job(dataset_name, seed, count_on, TP_on, ref_on, grid_per_dim)
